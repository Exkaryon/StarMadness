...
class celestialBody {
    ...
    interaction = 'all';                                        // Свойство указывает с какие методы взаимодействия проявляет объект во Вселенной. False - объект нейтрино; all - все возможные взаимодействия; kinetic - кинетические; physic - физических параметров; boundary - пересечение границ.
...
// Можно переделать метод колайдер, чтобы в кинетикрефлекшн передовались доп аргументы по условиям если health < 0 - id ибъектов, которые должны игнорировать рефлексию.
        this.init();
    }
